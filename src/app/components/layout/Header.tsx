      ...
      <div className="flex items-center gap-4">
        {isLogin ? (
          ...
          <div className="flex items-center gap-8">
            ...
            <Link
              href="/register"
              className="px-8 py-3 bg-green-400 text-black text-xs font-black uppercase tracking-widest rounded-full hover:scale-105 transition-all"
            >
              Join Studio
            </Link>
          </div>
      ...
      <AnimatePresence>
        {mobileMenu.isOpen && (
          <>
            ...
            >
              <button onClick={mobileMenu.close} className="self-end mb-12">
                <X size={32} />
              </button>
              <div className="space-y-6">
                {nav_links.map(({ label, href, icon: Icon }) => (
                  <Link
                    ...
                    href={href}
                    onClick={mobileMenu.close}
                    className="flex items-center gap-4 text-2xl font-heading italic uppercase"
                  >
...
const HeaderActionLink = ({
  ...
  <Link
    href={href}
    className="p-3 rounded-full hover:bg-white/5 text-zinc-400 hover:text-green-400 transition-all"
  >
    {icon}
  </Link>
);
